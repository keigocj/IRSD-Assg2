...
#pip install -U google-genai - this is to install the required packages
#pip install python-dotenv - this is to install the required packages
...
with st.sidebar:
    ...
    #check if th user upload the file and if there's a file call doc_ref
    if uploaded_file_ui and "doc_ref" not in st.session_state:
        ...
        with open("temp_doc", "wb") as f: #created a temp doc file with writing capability
...
for msg in st.session_state.messages: #iterate through the list of dictionaries
    with st.chat_message(msg["role"]): #create a div block for each role
        st.markdown(msg["content"]) #here we print the content
